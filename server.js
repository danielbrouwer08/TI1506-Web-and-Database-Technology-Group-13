...
server.get('/login/facebook/callback',
	...
	function (req, res) {
		// var query = "INSERT INTO user (Id,name) VALUES('" + req.user.facebookId + "','" + req.user.displayName + "')";
		// connection.query(query, function (error, results, fiels) {
		// 	console.log(error);
		// });
		// query = "INSERT INTO todolist (Id,Owner) VALUES('" + 1 + "','" + req.user.facebookId + "')";
		// connection.query(query, function (error, results, fiels) {
		// 	console.log(error);
		// });
...
server.get("/getTodo", function (req, res) {
	...
	//console.log("req:");
	//console.log(req);
	//var query = 'Select * From todoitem WHERE todoitem.DueDate is not null and todoitem.CompletionDate is not null';
	var query = 'Select * From todoitem WHERE todoitem.ItemOwner=' + req.user.facebookId + ' and todoitem.DueDate is not null and todoitem.CompletionDate is not null';
...
server.post("/saveTodo", function (req, res) {
	...
	var delquery = 'DELETE FROM todoitem where todoitem.id=' + temptodo.id + ' and todoitem.ItemOwner=' + req.user.facebookId;
	connection.query(delquery, function (error, results, fiels) {
		console.log("delquery errors:");
		console.log(error);
	});
	...
	var query = "INSERT INTO todoitem (Id,Title,Text,DueDate,CompletionDate,Priority,Completed,ToDoListID,ItemOwner) VALUES('" + temptodo.id + "','" + temptodo.subject + "','" + temptodo.extraInfo + "','" + dueDate + "','" + reminderDate + "','" + temptodo.priority + "','" + temptodo.done + "','" + 1 + "','" + req.user.facebookId + "')";
...
server.post("/deleteTodo", function (req, res) {
	...
	var query = 'DELETE FROM todoitem where todoitem.id=' + temptodo.id + ' and todoitem.ItemOwner=' + req.user.facebookId;
	connection.query(query, function (error, results, fiels) {
		console.log(error); //log any errors
	});
...
//Create an account on the server (NOT USED ANYMORE)
